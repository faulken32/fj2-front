...
type RestOf<T extends IJob | NewJob> = Omit<T, 'validUntil'> & {
...
export class JobService {
  ...
  protected convertDateFromClient<T extends IJob | NewJob | PartialUpdateJob>(job: T): RestOf<T> {
    return {
      ...job,
      validUntil: job.validUntil?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restJob: RestJob): IJob {
    return {
      ...restJob,
      validUntil: restJob.validUntil ? dayjs(restJob.validUntil) : undefined,
    };
  }
