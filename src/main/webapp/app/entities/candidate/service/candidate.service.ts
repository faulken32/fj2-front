...
type RestOf<T extends ICandidate | NewCandidate> = Omit<T, 'validUntil'> & {
...
export class CandidateService {
  ...
  protected convertDateFromClient<T extends ICandidate | NewCandidate | PartialUpdateCandidate>(candidate: T): RestOf<T> {
    return {
      ...candidate,
      validUntil: candidate.validUntil?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCandidate: RestCandidate): ICandidate {
    return {
      ...restCandidate,
      validUntil: restCandidate.validUntil ? dayjs(restCandidate.validUntil) : undefined,
    };
  }
